...
function main() {
    ...
    function check_network_freedom() {
        GM_xmlhttpRequest({
            method: 'GET',
            url: 'https://hunhun.appspot.com/api/ip/json/data.json',
            timeout: 2000,
            responseType: 'json',
            onload: response => {
    ...
    if (!!area) {
        // Harmonious button
        const harmonious_button = document.createElement('button');
        ...
        harmonious_button.addEventListener('click', function () {
            ...
            if (fastPostMessageContent.length >= 2) {
                let message = '免疫完成';
                show_dialog(message);
            }
        }, false);
